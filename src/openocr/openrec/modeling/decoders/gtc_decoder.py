...
class GTCDecoderTwo(nn.Module):
    ...
    def forward(self, x, data=None, return_feats=False):
        ...
        if return_feats:
            ctc_feats, ctc_pred = self.ctc_decoder(x_ctc, data=data, return_features=True)

            if self.training or self.infer_gtc:
                gtc_feats, gtc_pred = self.gtc_decoder(x_gtc.flatten(2).transpose(1, 2), data=data, return_features=True)
                return {'gtc_feats': gtc_feats, 'ctc_feats': ctc_feats}, {'gtc_pred': gtc_pred, 'ctc_pred': ctc_pred}
