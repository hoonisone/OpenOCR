...
class RecGTCMetric(object):
    ...
    def __call__(self, pred_label, batch=None, training=False, *args, **kwargs):
        if self.only_ctc:
            return self.ctc_metric(pred_label, batch, training=training)
    ...
    def get_metric(self, training=False):
        if self.only_ctc:
            return self.ctc_metric.get_metric(training=training)
        """
        return metrics {
                 'acc': 0,
                 'norm_edit_dis': 0,
